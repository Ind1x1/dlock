...
if [ "$(printf '%02d\n' "${CUR_PYTHON_VERSION}")" -le 36 ]; then
  ...
  pip3 install protobuf==$PROTOBUF_VERSION grpcio==$GRPCIO_VERSION grpcio-tools==$GRPCIO_TOOLS_VERSION -i https://pypi.tuna.tsinghua.edu.cn/simple
  ...
  pip3 install protobuf=="$PROTOBUF_VERSION_SRC" grpcio=="$GRPCIO_VERSION_SRC" grpcio-tools=="$GRPCIO_TOOLS_VERSION_SRC" -i https://pypi.tuna.tsinghua.edu.cn/simple
fi
