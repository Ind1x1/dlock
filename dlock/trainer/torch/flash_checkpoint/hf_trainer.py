...
from dlock.trainer.torch.flash_checkpoint.deepspeed import (
    AsyncCheckpointAgent,
)
from dlock.trainer.torch.flash_checkpoint.deepspeed_engine import (
    DeepSpeedCheckpointEngine,
)
from dlock.trainer.torch.flash_checkpoint.engine import CheckpointEngine
from dlock.trainer.torch.flash_checkpoint.full_ckpt_engine import (
...
class FlashCkptTrainer(Trainer):
    ...
    by the step int the file `dlock_latest.txt` in the `OUTPUT_DIR` of
    `TrainingArguments`.
    """
    ...
    def get_last_checkpoint(self):
        ...
        persistence may not finish. The step in the `dlock_latest.txt` is
    ...
    def _get_last_checkpoint_step(self):
        tracer_file = os.path.join(self.args.output_dir, "dlock_latest.txt")
