...
def get_pending_timeout():
    if _dlock_context.seconds_to_wait_pending_pod <= 0:
        return JobConstant.PENDING_NODE_TIMEOUT_DEFAULT_MIN
    return _dlock_context.seconds_to_wait_pending_pod
...
def reduce_timeout_pending_node_resource(node: Node):
    ...
    new_cpu = math.ceil(
        original_cpu / _dlock_context.factor_to_cut_pending_cpu
    )
    ...
    new_memory = math.ceil(
        original_memory / _dlock_context.factor_to_cut_pending_mem
    )
...
class TrainingNodeManager(object):
    ...
    def _get_pending_timeout(self):
        timeout = _dlock_context.seconds_to_wait_pending_pod
        if timeout <= 0:
            return 0
