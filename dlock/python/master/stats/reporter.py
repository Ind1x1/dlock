...
from dlock.proto import brain_pb2
from dlock.python.brain.client import GlobalBrainClient
from dlock.python.common.constants import ReporterType
from dlock.python.common.grpc import ModelInfo
from dlock.python.common.log import default_logger as logger
from dlock.python.common.singleton import Singleton
from dlock.python.master.stats.training_metrics import (
    DatasetMetric,
    RuntimeMetric,
    TrainingHyperParams,
)
...
class StatsReporter(metaclass=ABCMeta):
    ...
    def new_stats_reporter(cls, job_meta, reporter=None):
        ...
        elif reporter == ReporterType.DLOCK_BRAIN:
...
class BrainReporter(StatsReporter, Singleton):
    ...
    def report_job_resource(self, job_resource):
        ...
        Args:
            job_uuid: str, the unique id of the job which is usually
                the uuid in the job yaml of k8s.
            job_resource: dlock.python.master.resource.JobResource instance.
        """
        job_metrics = init_job_metrics_message(self._job_meta)
