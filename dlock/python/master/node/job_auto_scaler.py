...
from dlock.python.common.node import NodeResource
...
from dlock.python.master.node.worker import WorkerManager
from dlock.python.master.resource.job import (
    JobResource,
    JobResourceOptimizer,
)
from dlock.python.master.resource.optimizer import ResourcePlan
...
class PSTrainingAutoScaler(JobAutoScaler):
    ...
    def start_auto_scaling(self):
        ...
        if not self._autoscaling_started:
            ...
            if (
                not _dlock_context.auto_ps_enabled
                and not _dlock_context.auto_worker_enabled
            ):
                return
    ...
    def _periodic_optimize_running_resource(self):
        ...
        last_plan_time = 0
        opt_interval = _dlock_context.seconds_interval_to_optimize
        while True:
            if not self._autoscaling_started:
...
class AllreduceTrainingAutoScaler(JobAutoScaler):
    ...
    def start_auto_scaling(self):
        ...
        if not self._autoscaling_started:
            self._autoscaling_started = True
            if _dlock_context.auto_worker_enabled:
                threading.Thread(
