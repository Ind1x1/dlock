...
from dlock.trainer.constants.tf_constants import TFConstants
from dlock.trainer.tensorflow.executor.base_executor import BaseExecutor
from dlock.trainer.tensorflow.hooks.elastic_data_shard_report_hook import (
    ElasticDataShardReportHook,
)
from dlock.trainer.tensorflow.hooks.global_step_hook import GlobalStepHook
from dlock.trainer.tensorflow.util import common_util
from dlock.trainer.tensorflow.util.data_mapping_util import data_mapping
from dlock.trainer.tensorflow.util.dataset_util import DatasetUtil
from dlock.trainer.tensorflow.util.estimator_util import (
    ck_after_run,
    hook_estimator_call_model_fn,
)
from dlock.trainer.tensorflow.util.tf_patch_util import export_saved_model
from dlock.trainer.tensorflow.util.tf_version_util import is_tf_115
from dlock.trainer.util.log_util import default_logger as logger
from dlock.trainer.util.reflect_util import get_class

try:
    from dlock.python.elastic_agent.tensorflow.hooks import (
        ReportModelInfoHook,
    )
except Exception:
    logger.warning("fail to import dlock")
...
class EstimatorExecutor(BaseExecutor):
    ...
    def _prepare_estimator(self, need_context=False):
        ...
        logger.info(
            ...
            " dlock.trainer will decide the training and evaluting process."
        )
