...
from dlock import trainer
from dlock.trainer.constants.platform_constants import PlatformConstants
from dlock.trainer.mock.tf_process_scheduler import (
    TFProcessScheduler,
    mock_k8s_platform_subprocess,
    mock_ray_platform_subprocess,
)
from dlock.trainer.util.args_util import get_parsed_args
from dlock.trainer.util.log_util import default_logger as logger
from dlock.trainer.worker.tf_kubernetes_worker import TFKubernetesWorker
from dlock.trainer.worker.tf_ray_worker import TFRayWorker


def print_info(append_detail=False):
    """Print dlock_trainer information"""
    dlock_trainer = os.path.dirname(trainer.__file__)
    file_path = os.path.join(dlock_trainer, "COMMIT_INFO")
