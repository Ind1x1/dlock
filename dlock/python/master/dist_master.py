...
from dlock.python.master.elastic_training.elastic_ps import ElasticPsService
...
from dlock.python.master.master import JobMaster
from dlock.python.master.monitor.error_monitor import ErrorMonitor
from dlock.python.master.monitor.speed_monitor import SpeedMonitor
from dlock.python.master.node.dist_job_manager import create_job_manager
from dlock.python.master.node.event_callback import (
    AllReduceNodeHandlingCallback,
    TaskRescheduleCallback,
    TFPSNodeHandlingCallback,
)
from dlock.python.master.servicer import create_master_service
...
def _create_master_service_on_k8s(namespace, job_name, job_uuid, target_port):
    from dlock.python.scheduler.kubernetes import (
        NODE_SERVICE_PORTS,
        k8sClient,
        k8sServiceFactory,
    )
    ...
    svc_name = f"elasticjob-{job_name}-dlock-master"
    port = NODE_SERVICE_PORTS[NodeType.DLOCK_MASTER]
    selector = {
        ElasticJobLabel.JOB_KEY: job_name,
        ElasticJobLabel.REPLICA_TYPE_KEY: NodeType.DLOCK_MASTER,
    }
...
class DistributedJobMaster(JobMaster):
    ...
    def _create_metric_collector_if_needed(self, params: JobArgs):
        ...
        if params.optimize_mode == OptimizeMode.CLUSTER:
            reporter = ReporterType.DLOCK_BRAIN
