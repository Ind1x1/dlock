...
from dlock.python.common.constants import CheckpointConstant
from dlock.python.common.log import default_logger as logger
from dlock.python.common.multi_process import (
    LocalSocketComm,
    SharedLock,
    SharedQueue,
)
from dlock.python.common.singleton import Singleton
from dlock.python.common.storage import CheckpointStorage
from dlock.python.elastic_agent.torch.ckpt_saver import (
    DLOCK_CKPT_CONFIG_KEY,
...
from dlock.trainer.torch.flash_checkpoint.replica import CkptReplicaManger
...
def start_saver_process():
    ...
    process is not launched by `dlock-run`. This process will
    exit and cannot save the checkpoint after the training process exit.
    It is better to use `dlock-run` to start the training process.
    `dlock-run` can save checkpoint once the training process fails
    ...
    # if the training process is not launched by dlock-run.
    if role_name != "dlock-trainer" and local_rank == 0:
...
class CheckpointEngine(metaclass=ABCMeta):
    ...
    def save_state_dict_to_memory(self, state_dict, conf: CheckpointConfig):
        ...
        if not all_rank_ready or not state_dict:
            ...
            return False
        state_dict[DLOCK_CKPT_CONFIG_KEY] = conf
    ...
    def get_state_dict_from_memory(self):
        ...
        if passed and config.step > 0:
            ...
            state_dict.pop(DLOCK_CKPT_CONFIG_KEY, None)
