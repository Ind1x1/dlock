...
from dlock.python.common import env_utils
...
from dlock.python.common.error import ProcessError
from dlock.python.common.grpc import (
...
from dlock.python.elastic_agent.config.paral_config_tuner import (
    ParalConfigTuner,
)
from dlock.python.elastic_agent.context import get_agent_context
from dlock.python.elastic_agent.diagnosis.diagnosis_agent import (
    DiagnosisAgent,
)
from dlock.python.elastic_agent.master_client import MasterClient
from dlock.python.elastic_agent.monitor.training import TorchTrainingMonitor
from dlock.python.elastic_agent.torch.ckpt_saver import AsyncCheckpointSaver
from dlock.python.elastic_agent.torch.master_kv_store import MasterKVStore
from dlock.python.util.numa_util import get_gpu_affinity, get_npu_affinity
from dlock.python.util.time_util import timestamp_diff_in_seconds
from dlock.trainer.torch.utils import (
...
class ElasticTrainingAgent(LocalElasticAgent):
    ...
    will fail and the dlock will relaunch the node. Because, we find
    ...
    def _stop_workers_to_restart(self):
        """
        The agent query from the dlock job master to check whether to restart
...
def run_network_check(config: ElasticLaunchConfig, entrypoint):
    if config.accelerator == Accelerators.NVIDIA_GPU:
        cmd_args = ["-m", "dlock.trainer.torch.node_check.nvidia_gpu"]
    elif config.accelerator == Accelerators.ASCEND_NPU:
        cmd_args = ["-m", "dlock.trainer.torch.node_check.ascend_npu"]
    else:
    ...
    for _ in range(2):
        ...
        # dlock will replace the abnormal node with a new node.
