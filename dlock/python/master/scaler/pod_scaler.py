...
class PodScaler(Scaler):
    ...
    def _get_master_addr(self):
        svc_name = f"elasticjob-{self._job_name}-dlock-master"
        port = NODE_SERVICE_PORTS[NodeType.DLOCK_MASTER]
        master_addr = f"{svc_name}:{port}"
        target_port = _dlock_context.master_port
        if not self._check_master_service_avaliable(svc_name, target_port):
            ...
            master_addr = f"{master_ip}:{_dlock_context.master_port}"
        return master_addr
    ...
    def _init_pod_config_by_job(self):
        ...
        if "replicaSpecs" in self._job["spec"]:
            for replica, spec in self._job["spec"]["replicaSpecs"].items():
                if replica == NodeType.DLOCK_MASTER:
                    continue
                pod = spec["template"]
                pod["apiVesion"] = "v1"
                pod["kind"] = "Pod"
    ...
    def _list_job_pods(self):
        ...
        for pod in pod_list.items:
            ...
            if pod_type == NodeType.DLOCK_MASTER:
                continue
    ...
    def _create_pod(self, node: Node):
        ...
        env.append(
            V1EnvVar(name=NodeEnv.DLOCK_MASTER_ADDR, value=self._master_addr)
        )
