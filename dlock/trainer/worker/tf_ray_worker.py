...
from dlock.python.common.constants import NodeEventType
...
from dlock.trainer.tensorflow.executor.estimator_executor import (
    EstimatorExecutor,
)
from dlock.trainer.tensorflow.failover.tensorflow_failover import (
    TensorflowFailover,
)
from dlock.trainer.tensorflow.util import common_util
from dlock.trainer.util.conf_util import get_conf
...
class TFRayWorker:
        ...
        if self._args.get("mock"):
            ...
            while True:
                ...
                logger.info(
                    ...
                    "report their service_addr to dlock master"
                )
        return ps_cluster
    ...
    def report_ps_address(self, address):
        if self._args.get("mock"):
            ...
            # if not mock, the ps report its service_addr to dlock master
            ps_nodes, _ = master_client.query_ps_nodes()
            logger.info("ps nodes is {}".format(ps_nodes))
